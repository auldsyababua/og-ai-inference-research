# Prepare skeleton content for research-findings.md based on gathered data so far

content = '''# GPU–Generator Stability Integration: Initial Research Findings
...
For each genset, the inertia constant \(H\) in seconds is approximated from rotor inertia \(J\) using the standard relationship for synchronous machines:\[ H = \frac{J \omega^2}{2 S_\text{base}} \]

Where:
- \(J\): mass moment of inertia \(\text{kg·m}^2\)
- \(\omega\): mechanical angular speed (rad/s) at synchronous speed
- \(S_\text{base}\): MVA rating \(\approx\) MW at pf = 1.0

Synchronous speed and pole count are inferred from rated frequency and rpm. This is consistent with utility references on inertia constants and per-unit formulation. Confidence in the *form* of the equation is very high (\>95%).[44][45][51][149]
...
Multiple grid-operator and academic references give characteristic inertia constant ranges:\[ H_\text{typ} \approx 2\text{–}7\,\text{s} \]
...
- **Conservative “no‑trip” RoCoF design target**: \(\lvert\text{RoCoF}\rvert \le 0.5\,\text{Hz/s}\) (high safety margin vs many codes)[52][55][61][117][120]
- **Absolute worst‑case threshold for short events**: \(\lvert\text{RoCoF}\rvert \le 1.0\,\text{Hz/s}\) over 500 ms (aligned with modern RoCoF ride‑through capability expectations).[52][55][61][117]
...
- **Tight limit** (GREEN zone target): \(\lvert\Delta f\rvert \le 0.5\,\text{Hz}\)
- **Upper bound for YELLOW**: \(\lvert\Delta f\rvert \le 1.0\,\text{Hz}\)
...
1. **Single‑cluster step (100 PCIe H100 GPUs)**
   ...
   - For modeling, treat **idle → full‑load step** as **ΔP_gpu ≈ 250 W** (100 → 350 W) with 50–65% confidence until direct H100 PCIe measurements are available.
...
2. **Sustained inference load level**
   ...
   - Translating this to PCIe inference is non‑trivial, but as a conservative initial estimate, assume **sustained inference at 70–80% of 350 W**, i.e. **245–280 W per GPU**.[155][156][164][168]
...
1. **Cluster step power**:\[ \Delta P_\text{cluster} = C \cdot N \cdot \Delta P_\text{gpu} \]
2. **RoCoF estimate** (per generator):\[ \text{RoCoF} = - \frac{\Delta P_\text{cluster}}{2 H_\text{eff} S_\text{base}} \]
3. **Steady‑state frequency deviation** (droop mode):\[ \frac{\Delta f}{f_\text{nom}} \approx - R_\text{eff} \cdot \frac{\Delta P_\text{cluster}}{P_\text{rated}} \]
...
4. **Formalize parameter bands for Phase 3 modeling**
   ...
   - For GPUs, lock **ΔP_gpu** and event timing bands pending measurement (e.g., best guess + conservative margins).
